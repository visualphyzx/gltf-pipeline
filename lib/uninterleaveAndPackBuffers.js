...
function packGltfBuffer(gltf, bufferId, packBufferViews) {
    ...
    var packBuffer = new Uint8Array(source.length + 6); // Extra space for padding between targets
    ...
    for (var i = 0; i < targets.length; i++) {
        ...
        if (i < targets.length - 1) {
            // Align on a 4-byte boundary so the byteOffset of the next target's first accessor is 0
            offset += offset % 4;
        }
    }
